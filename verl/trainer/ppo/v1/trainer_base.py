...
class PPOTrainer(ABC):
    ...
    def fit(self, agent_loop_manager: AgentLoopManager):
        ...
        while current_epoch < self.config.trainer.total_epochs and self.global_steps <= self.total_training_steps:
            ...
            # PATCH(liam): replayed batches stay in storage for reuse; the
            # replay sampler owns their lifecycle (evicts + clears itself).
            if not ((batch.extra_info or {}).get("reuse_replay", False) or OmegaConf.select(self.config, "trainer.v1.sampler.sampler_kwargs.reuse_replay", default=False)):
    ...
    def _validate(self) -> dict[str, float]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            # PATCH(liam): see step-loop clear guard — replayed batches are
            # cleared by the replay sampler, not per-step.
            if not ((batch.extra_info or {}).get("reuse_replay", False) or OmegaConf.select(self.config, "trainer.v1.sampler.sampler_kwargs.reuse_replay", default=False)):
                tq.kv_clear(keys=batch.keys, partition_id=batch.partition_id)
    ...
    def _add_batch_to_generate(self):
        ...
        # after max_fresh_batches; later steps run purely from the replay pool
        # instead of re-coupling to generation via dispatch backpressure.
        if OmegaConf.select(self.config, "trainer.v1.sampler.sampler_kwargs.reuse_replay", default=False):
            cap = int(OmegaConf.select(self.config, "trainer.v1.sampler.sampler_kwargs.max_fresh_batches", default=3))
            done = int(getattr(self, "_liam_fresh_batches", 0))
            if done >= cap:
                return
