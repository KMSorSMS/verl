...
class PPOTrainerSync(PPOTrainer):
    ...
    def _update_actor(self, batch: KVBatchMeta, metrics: dict) -> KVBatchMeta:
        if float(self.config.algorithm.distill_kl_coef) > 0 and batch.fields is not None:
            # Preserve sync's curated actor field selection; the base expands None to that exact list.
            batch = KVBatchMeta(
                keys=batch.keys,
                tags=batch.tags,
